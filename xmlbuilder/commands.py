...
class ChangeValueCommand(QUndoCommand):
    def __init__(self, row, column, value, model):
        ...
        self.column = column
        self.model = model
        self.setText("Row %s: Set %s = '%s'"%(row + 1, model._header[column].toString(), value.toString()))
    ...
    def _do(self, new, old):
        ...
        if self.column == 0:
            ...
            # we have just changed the object name, other objects might reference it
            index1 = self.model.index(0, 0)
            index2 = self.model.index(self.model.rowCount()-1, self.model.columnCount()-1)
        else:
...
class RowCommand(QUndoCommand):
    ...
    def addRow(self):
        ...
        self.model.rows = self.model.rows[:self.row] + [self.rowdata] + self.model.rows[self.row:]
        self.model.endInsertRows()

    def removeRow(self):
        self.model.beginRemoveRows(self._parent, self.row, self.row)
        self.model.rows = self.model.rows[:self.row] + self.model.rows[self.row + 1:]
